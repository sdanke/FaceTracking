    ...
    for (TRACKER_DATA& data : tid_feats) {
        ...
        if (samples.find(track_id) != samples.end()) { // append
            ...
            if (newSize <= this->budget) {
                FEATURESS newSampleFeatures(newSize, ReidFeatureDims);
                newSampleFeatures.block(0, 0, oldSize, ReidFeatureDims) = samples[track_id];
                newSampleFeatures.block(oldSize, 0, addSize, ReidFeatureDims) = newFeatOne;
                samples[track_id] = newSampleFeatures;
            } else {
                if (oldSize < this->budget) { // original space is not enough;
                    FEATURESS newSampleFeatures(this->budget, ReidFeatureDims);
                    if (addSize >= this->budget) {
                        newSampleFeatures = newFeatOne.block(0, 0, this->budget, ReidFeatureDims);
                    } else {
                    ...
                    if (addSize >= this->budget) {
                        samples[track_id] = newFeatOne.block(0, 0, this->budget, ReidFeatureDims);
                    } else {
